...
export const api = {
  ...
  ssh: {
    getAccessStatus: async () => {
      // enabledUsers count is now computed server-side; pass 0 as fallback
      return apiRequest<{ blocked: boolean; reason?: string }>(`/api/ssh/access-status?enabledUsers=0`);
    },
  },
  servers: {
  ...
  admin: {
    ...
    sshPolicies: {
      ...
      commit: (id: string, signature?: string) =>
        apiRequest<{ success: boolean; syncData?: any }>(`/api/admin/ssh-policies/pending/${id}/commit`, {
          method: "POST",
    ...
    license: {
      get: () => apiRequest<LicenseInfo>(`/api/admin/license?totalUsers=0&enabledUsers=0`),
